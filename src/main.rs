...
fn solve_naive(board: Board, mut alpha: i8, beta: i8, passed: bool,
    ...
    while empties != 0 {
        ...
        match board.play(pos) {
            Ok(next) => {
                pass = false;
                res = max(res, -solve(
                        next, -beta, -alpha, false, table, table_order, count));
                alpha = max(alpha, res);
                if alpha >= beta {
                    return res;
                }
    ...
    if pass {
        if passed {
            ...
            return -solve(board.pass(), -beta, -alpha, true, table, table_order, count);
        }
    }
    res
}
...
fn solve_fastest_first(board: Board, mut alpha: i8, beta: i8, passed: bool,
    ...
    for (i, &(_, ref next)) in v.iter().enumerate() {
        if i == 0 {
            res = max(res, -solve(
                    next.clone(), -beta, -alpha, false, table, table_order, count));
        } else {
            let mut result = -solve(
                next.clone(), -alpha-1, -alpha, false, table, table_order, count);
            if result >= beta {
                return result;
            }
            if result > alpha {
...
fn solve_with_table(board: Board, alpha: i8, beta: i8, passed: bool,
    ...
    let res = solve_move_ordering_with_table(
        board.clone(), alpha, beta, passed, table, table_order, count);
...
fn solve(board: Board, alpha: i8, beta: i8, passed: bool,
    ...
    *count += 1;
    if popcnt(board.empty()) == 0 {
        ...
        solve_with_table(board, alpha, beta, passed, table, table_order, count)
    }
}
    ...
    for (i, line) in reader.lines().enumerate() {
        match Board::from_str(&line.unwrap()) {
            Ok(board) => {
                ...
                let mut count = 0;
                let res = solve(
                    board, -64, 64, false, &mut table, &table_order, &mut count);
